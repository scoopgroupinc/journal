...
function Water({ cups, water, setWater }: { cups: number; water: number; setWater: any }) {
  const count = [...Array(cups).keys()];
  return (
    <div className="flex items-center space-x-2">
      {count.map((cup, index) => (
        <div className="w-8" key={index} onClick={() => setWater(index)}>
          <svg
            className={`w-8 h-8 ${
              index <= water ? "text-cyan-200" : "dark:text-gray-500 opacity-30"
            }`}
            aria-hidden="true"
